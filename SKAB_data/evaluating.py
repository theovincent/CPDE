...
def evaluating_change_point(true, prediction, metric="nab", numenta_time=None):
            ...
            for couple in detecting_boundaries:
                ...
                if prediction[t1:t2].sum() == 0:
                    ...
                    detectHistory.append(
                        prediction[prediction == 1][t1:t2].index[0] - t1
                    )
            return missing, detectHistory

        if type(prediction) != type(list()):
            missing, detectHistory = single_average_delay(
                detecting_boundaries, prediction
            )
        else:
            missing, detectHistory = 0, []
            for i in range(len(prediction)):
                missing_, detectHistory_ = single_average_delay(
                    detecting_boundaries[i], prediction[i]
                )
                missing, detectHistory = (
                    missing + missing_,
    ...
    def evaluate_nab(detecting_boundaries, prediction, table_of_coef=None):
        ...
        def single_evaluate_nab(
            detecting_boundaries, prediction, table_of_coef=None, name_of_dataset=None
        ):
            ...
            for profile in ["Standart", "LowFP", "LowFN"]:
                ...
                for i in range(len(alist)):
                    ...
                    if win_fault_slow.sum() == 0:
                        ...
                        tr = pd.Series(
                            win_fault_slow.values,
                            index=range(
                                -len(win_fault), len(win_fault_slow) - len(win_fault)
                            ),
                        )
        ...
        if type(prediction) != type(list()):
            matrix = single_evaluate_nab(
                detecting_boundaries, prediction, table_of_coef=table_of_coef
            )
        else:
            matrix = np.zeros((3, 3))
            for i in range(len(prediction)):
    ...
    if not metric == "binary":
        ...
        def single_detecting_boundaries(true, numenta_time, true_items):
            ...
            td = (
                pd.Timedelta(numenta_time)
                if numenta_time is not None
                else pd.Timedelta((true.index[-1] - true.index[0]) / len(true_items))
            )
        ...
        if type(true) != type(list()):
            detecting_boundaries = single_detecting_boundaries(
                true=true, numenta_time=numenta_time, true_items=true_items
            )
        else:
            detecting_boundaries = []
            for i in range(len(true)):
